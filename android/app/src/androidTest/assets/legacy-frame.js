      function frame(time) {
        ...
        const g = game || fresh(),
          cx = width / 2,
          top = Math.max(140, hud.offsetHeight || 140),
          bottom = height - 150,
        ...
        sky.addColorStop(0.58, "#070b17");
        sky.addColorStop(1, "#010207");
        ctx.fillStyle = sky;
        ctx.fillRect(0, 0, width, height);
        for (let b = 0; b < 3; b++) {
          ctx.save();
          ctx.globalAlpha = 0.055 + b * 0.018;
          ctx.strokeStyle = LASERS[dailyTrack?.palette?.[(stage + b) % 5] ?? (stage + b) % 5][0];
          ctx.lineWidth = 70 + b * 24;
          ...
          for (let x = -60; x <= width + 60; x += 12) {
            const y =
              ...
              b * 170 +
              Math.sin(
                x * 0.012 + b * 1.7 + (settings.reduced ? 0 : time * 0.00012),
              ) *
                55;
